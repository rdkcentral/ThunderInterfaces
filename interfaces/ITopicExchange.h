...
namespace Exchange {
    ...
    struct ITopicExchange : virtual public Core::IUnknown {
        ...
        enum { ID = ID_TOPICEXCHANGE_TOPIC_MSGNOTIFICATION };
                ...
                // @statuslistener (note will not work on 4.4)
                virtual void Message(const string& topicID /* @index */,  const string& payload, const payloadtype payloadtype) = 0;
        };

        virtual Core::hresult Register(const string& topicID, IMsgNotification* const sink) = 0;
        virtual Core::hresult Unregister(const string& topicID, const IMsgNotification* const sink) = 0;
